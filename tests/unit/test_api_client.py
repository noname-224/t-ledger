...
from t_ledger.infra.api.mappers.core import parse_account, parse_portfolio, parse_bonds, \
    parse_bonds_with_coupons


@pytest.mark.asyncio
async def test_request_passed(api_client, mocked_request_params):
    data = await api_client._request(*mocked_request_params(
        status=200,
        json_data={"test": "test"}
    ))
...
async def test_request_failed(api_client, mocked_request_params):
    with pytest.raises(ApiClientRequestError):
        await api_client._request(*mocked_request_params(
            status=400,
            json_data={"test": "test"}
        ))
...
@pytest.mark.parametrize("portfolio_data", [
    {
        "positions": [],
        "accountId": "123456789",
...
@pytest.mark.parametrize("bonds_data,bonds_uids", [
    (
        [{"instrument": {}}],
        ["uid", "uid"],
    ),
...
@pytest.mark.parametrize("coupons_data,bonds_uids", [
    (
        [
            {
                "events": [
